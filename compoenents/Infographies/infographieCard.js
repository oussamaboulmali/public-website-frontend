...
const InfographieCard = ({ infographie, envConfig }) => {
  const [imageError, setImageError] = useState(false);
  const hasImage = infographie?.image?.url && !imageError;
  ...
  // URL de l'infographie
  const infographieUrl = `/infographie/${infographie?.alias?.replace(
    /^\/+/,
    ""
  )}`;
  ...
  return (
    ...
    >
      <div className="w-full h-full mx-auto bg-white rounded-[4px] overflow-hidden transition-transform duration-300 hover:shadow-lg hover:scale-102 shadow-xs flex flex-col cursor-pointer">
        {/* Image de infographie || placeholder image */}
          ...
          {hasImage ? (
            <>
              <Image
                ...
                width={429}
                height={200}
                alt={infographie?.image?.description || infographie?.title}
...
export default InfographieCard;
