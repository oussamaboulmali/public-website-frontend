...
const CahierCard = ({ cahier, envConfig }) => {
  const [imageError, setImageError] = useState(false);
  const hasImage = cahier?.featuredImage?.url && !imageError;
  ...
  // URL du cahier multimédia
  const cahierUrl = `/cahier-multimedia/${cahier?.alias?.replace(/^\/+/, "")}`;

  return (
    <Link href={cahierUrl} className="block w-full h-full" prefetch={false}>
      <div className="w-full h-full mx-auto bg-white rounded-[4px] overflow-hidden transition-transform duration-300 hover:shadow-lg hover:scale-102 shadow-xs flex flex-col cursor-pointer">
        {/* Image de cahier || placeholder image */}
          ...
          {hasImage ? (
            <>
              <Image
                ...
                width={429}
                height={200}
                alt={cahier?.featuredImage?.description || cahier?.title}
