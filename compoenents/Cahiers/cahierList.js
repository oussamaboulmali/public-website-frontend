...
import { paginateCahierAction } from "src/app/actions/actions-server";
...
export default function CahierList({
  ...
  const [array, setArray] = useState(cahier || []);
  const [currentPage, setCurrentPage] = useState(1);
  const totalPages = Math.ceil((count || cahier?.length || 0) / pageSize);
  const [statePaginate, formAction] = useActionState(
    paginateCahierAction,
    null
  );
  ...
  return (
    <div className="w-full  mx-auto h-fit min-h-screen  sm:px-1 lg:px-8 pt-0  pb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-6 ">
        {array?.map((cahier, index) => (
          <CahierCard key={index} cahier={cahier} envConfig={envConfig} />
        ))}
      </div>
