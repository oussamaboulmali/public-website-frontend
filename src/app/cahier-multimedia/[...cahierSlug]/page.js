...
import CahierContent from "../../../../compoenents/Cahiers/cahierContent";
...
export async function generateMetadata({ params }) {
  ...
  const slugs = paramss?.cahierSlug ?? [];

  // Appel API pour récupérer les données de l'article
  const response = await fetchData(
    envConfig.baseUrl + "cahiers/detail",
    "POST",
    {
      alias: slugs[0],
    }
  ...
  const canonical = `${domain}/${
    envConfig.lang
  }/cahier-multimedia/${slugs?.join("/")}`.replace(/\/+$/, "");
...
export default async function Page({ params }) {
  ...
  const slug = paramsValue?.cahierSlug;
  const cahierDetails = await fetchData(
    envConfig.baseUrl + "cahiers/detail",
    "post",
    {
      alias: slug[0],
  ...
  let updatedSlugs = [
    ...
    { path: slug[0], label: cahierDetails?.data?.data?.name },
  ];

  // Vérifiez status api
  if (cahierDetails.status >= 400 && cahierDetails.status < 500) {
    notFound();
  }

  if (cahierDetails.status >= 300 && cahierDetails.status < 400) {
    redirect(cahierDetails?.data?.data?.alias);
  }

  if (cahierDetails.status >= 500) {
  ...
  return (
    <div className="w-[90%] mx-auto">
      ...
      <CahierContent
        cahier={cahierDetails?.data?.data}
        envConfig={envConfig}
        dir={dir}
        alias={slug[0]}
      />
